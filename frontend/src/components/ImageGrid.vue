...
function openPreview(image) { current.value=image;metadata.value={alt:image.alt||'',author:image.author||'',license:image.license||'',tags:(image.tags||[]).join(', ')};preview.value=true; }
</script>
<template>
  <div v-if="selectable && session.admin" class="gallery-selection"><span>已选择 {{ selected.length }} 张</span><UiSelect v-model="selectedVisibility" aria-label="批量可见性" :options="[{label:'公开',value:'public'},{label:'不列出',value:'unlisted'},{label:'真正私有',value:'private'}]" /><UiButton :disabled="!selected.length" @click="updateSelectedVisibility">设置可见性</UiButton><UiButton icon="download" :disabled="!selected.length" @click="exportSelected">导出 ZIP</UiButton><UiButton :disabled="!selected.length" variant="danger" @click="batchRemove">删除所选</UiButton></div>
  ...
  <UiDialog v-model:visible="preview" title="图片预览" width="1000px"><img v-if="current" class="preview-image" :src="current.url" :alt="current.alt || current.originalName"><p class="field-help">{{ current?.originalName }}</p><div v-if="selectable && session.admin" class="image-metadata-form"><label>替代文字<UiInput v-model="metadata.alt" maxlength="500" /></label><label>作者<UiInput v-model="metadata.author" maxlength="200" /></label><label>授权方式<UiInput v-model="metadata.license" maxlength="200" /></label><label>标签（以逗号分隔）<UiInput v-model="metadata.tags" maxlength="500" /></label><UiButton variant="primary" @click="saveMetadata">保存信息</UiButton></div></UiDialog>
</template>
