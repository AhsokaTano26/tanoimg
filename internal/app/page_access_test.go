...
func TestPageAccessAndFrontendAssets(t *testing.T) {
	...
	for _, target := range []string{"/admin", "/admin/gallery", "/admin/upload", "/admin/recycle", "/admin/stats", "/admin/api", "/admin/settings", "/settings", "/recycle", "/stats", "/api"} {
		got := adminRequest(a, "", http.MethodGet, target, "")
	...
	for _, entry := range entries {
		...
		if got.Code != 200 || !strings.Contains(got.Header().Get("Content-Type"), "javascript") {
