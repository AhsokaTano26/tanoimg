...
func (a *App) Handler() http.Handler {
	...
	mux.HandleFunc("GET /api/auth/methods", a.authHandler(func(w http.ResponseWriter, r *http.Request) {
		ok(w, map[string]any{"passkeyAvailable": a.webAuthn != nil})
	}))
	mux.HandleFunc("GET /api/admin/security", a.authHandler(a.securityStatus))
	mux.HandleFunc("POST /api/auth/totp", a.authHandler(a.totpLogin))
	mux.HandleFunc("POST /api/admin/totp/setup", a.authHandler(a.totpSetup))
	mux.HandleFunc("POST /api/admin/totp/enable", a.authHandler(a.totpEnable))
	mux.HandleFunc("DELETE /api/admin/totp", a.authHandler(a.totpDisable))
	mux.HandleFunc("POST /api/admin/recovery-codes", a.authHandler(a.rotateRecovery))
	mux.HandleFunc("POST /api/admin/passkeys/register/begin", a.authHandler(a.passkeyRegisterBegin))
	mux.HandleFunc("POST /api/admin/passkeys/register/finish", a.authHandler(a.passkeyRegisterFinish))
	mux.HandleFunc("DELETE /api/admin/passkeys/{id}", a.authHandler(a.passkeyDelete))
	mux.HandleFunc("POST /api/auth/passkey/begin", a.authHandler(a.passkeyLoginBegin))
	mux.HandleFunc("POST /api/auth/passkey/finish", a.authHandler(a.passkeyLoginFinish))
	mux.HandleFunc("POST /api/auth/logout", a.logout)
	mux.HandleFunc("GET /api/auth/verify", a.verify)
	mux.HandleFunc("PUT /api/admin/password", a.authHandler(a.changePassword))
